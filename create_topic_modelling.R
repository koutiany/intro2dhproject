...
himym_collocations <- quanteda.textstats::textstat_collocations(corpus_tokens, min_count = 4)
...
dim(DTM)
#-----------------------------------------------------------------------
top10_terms <- c("ted", "lily", "marshall", "barney", "robin", "claudia", "zoey", "stella", "karen", "jenkins", "robin's", "natalie", "nora", "bob", "mike")
...
ggsave('allSeasons.jpeg', device = "jpeg", limitsize = FALSE)
...
#max distribution
theta1 <- theta %>% as_tibble %>% mutate(speaker = textdata$speaker) %>% group_by(speaker) %>% summarise(across(where(is.numeric), ~ max(.x, na.rm = TRUE)))
theta2 <- melt(theta1)
selectednames <- c("robin","lily","barney","ted","marshall")
theta3 <- filter(theta2, speaker %in% selectednames)
...
ggplot()+
  geom_point(data=theta3, mapping=aes(y=value,x=variable,color=speaker,shape=speaker))+
  labs(x="topic",y="theta")
ggsave('allSeasons_max.jpeg', device = "jpeg", limitsize = FALSE)
